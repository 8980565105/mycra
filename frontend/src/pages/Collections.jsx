...
function Collections({ products = [] }) {
  ...
  // FIX 1: Read BOTH ?category=Cars (name, from Home page) AND ?categoryId=xxx (id, internal)
  ...
  // FIX 1: Auto-select from URL — supports both ?category=Cars and ?categoryId=xxx
  useEffect(() => {
    if (!categories || categories.length === 0) return;

    if (categoryIdFromUrl) {
      // Internal click → match by _id
      const found = categories.find(
        (cat) => String(cat._id) === String(categoryIdFromUrl),
      );
      setSelectedCategory(found || null);
    } else if (categoryNameFromUrl) {
      // From Home page → match by name (case-insensitive)
      const found = categories.find(
        (cat) => cat.name.toLowerCase() === categoryNameFromUrl.toLowerCase(),
      );
      setSelectedCategory(found || null);
    } else {
      // No param → show all subcategories
  ...
  // Parent categories for main grid
  const parentCategories =
    !catLoading && categories && categories.length > 0
      ? categories.filter((cat) => cat.parent_id === null)
      : STATIC_CATEGORIES;

  // FIX 2: Handle both populated { _id, name } and plain string parent_id
