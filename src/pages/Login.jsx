...
const LoginForm = ({ onClose, onSwitchRegister, onSwitchForget }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // domain hatavi didhu — customer account global chhe
    const loginData = {
      email: formData.email,
      password: formData.password,
    };
      ...
      <div className="flex items-center justify-center">
        <div className="bg-white box-shadow rounded-lg flex overflow-hidden w-full max-w-[1062px] mx-auto">
          ...
          <div className="w-full md:w-2/3 px-5 py-5 sm:py-16 relative md:mx-0 md:px-20">
            ...
            <div className="mb-6 text-center">
              <img src={dynamicLogoUrl || HeaderLogo} alt="Logo" className="mx-auto mb-6" />
              <h3 className="mb-11 text-[var(--secondary-color)]">
                Welcome to {settings?.name || "maycra store"}
              </h3>
              <h3 className="text-[var(--primary-color)] text-bold text-[26px]">
            ...
            <div className="text-center mt-[40px] text-p">
              <button onClick={onSwitchRegister} className="text-theme underline">
                Create Account
              </button>
            </div>
          </div>
